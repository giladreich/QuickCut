...
function Component() {
    ...
    if (installer.isInstaller()) {
        ...
        // set target path
        var destInstallPath = Dir.toNative("@ApplicationsDirX86@\\QuickCut");
...
installMissingRedistributables = function () {
    ...
    var regKey = "HKEY_LOCAL_MACHINE\\SOFTWARE\\Wow6432Node\\Microsoft\\VisualStudio\\14.0\\VC\\Runtimes\\x86";
    ...
    if (needInstall) {
        ...
        installer.execute(Dir.toNative("@TargetDir@/vc_redist.x86.exe"), new Array("/repair", "/passive", "/norestart"));
